...
def get_largest_companies_fmp(limit=50):
    ...
    screening_url = f"https://financialmodelingprep.com/api/v3/stock-screener?marketCapMoreThan=1000000000&limit=500&apikey={api_key}"
...
def get_top_50_tickers(project_id: str):
    ...
    companies_df = get_largest_companies_fmp(limit=50)
    
    if companies_df is not None:     
        # Extract just the ticker symbols
        if 'symbol' in companies_df.columns:
            ticker_list = companies_df['symbol'].tolist()
            return ticker_list
        else:
