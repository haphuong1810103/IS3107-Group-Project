...
DATA_DIR = 'yfinance_minutes_data_json/'

BQ_DATASET = 'market_data'
BQ_TABLE = 'yf_minutes_json'
...
def get_latest_data_date(ticker):
    """Find the latest datetime for which we have data for the given ticker."""
    ...
    if not blob.exists():
        return (datetime.datetime.now(datetime.timezone.utc) - datetime.timedelta(days=5)) 
    ...
    return max(datetimes) + datetime.timedelta(minutes=15) 
...
def run_pipeline():
    current_date = datetime.datetime.now(datetime.timezone.utc)  # <-- CHANGED
    start_limit = current_date - datetime.timedelta(days=5)
